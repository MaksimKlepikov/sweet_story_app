 ...
 */
class GameField {
    constructor(type, currentLevel, game, x, y, width, height, rows, columns, mask) {
        if (type === 'sweetHill') {
    ...
    createSignals() {
        ...
        // // this.signals.applyBonusBomb.add(function (bonusBomb) {
        // //     console.log(bonusBomb.pos.x, bonusBomb.pos.y)
        // //     for (let i = 0; i < this.tileGrid.length; i++) {
        // //         if (this.tileGrid[i][bonusBomb.pos.y]) {
        // //             this.signals.removeTile.dispatch(this.tileGrid[i][bonusBomb.pos.y]);
        // //         }
        // //     }
        // //     for (let j = 0; j < this.tileGrid.length; j++) {
        // //         if (this.tileGrid[bonusBomb.pos.x][j]) {
        // //             this.signals.removeTile.dispatch(this.tileGrid[bonusBomb.pos.x][j]);
        // //         }
        // //     }
        // }, this);
        ...
        this.signals.tilesCrossOut.add(function (tileType) {
            //вызывается 2,3,... раза для строк длины 5,6,... соответственно
        ...
        if (this.gameType === 'sweetHill') {
            this.game.signals.isProgress.add(function (hillName, done) {
                if (this.currentLevel.name === hillName) {
                    // console.log(this.currentLevel.items.length, done)
                    if (done < this.currentLevel.items.length) {
                        this.currentLevel.currentTile = this.currentLevel.items[done]
                        // console.log(this.currentLevel.currentTile.name,'current tile')
                    }
                    else {
    ...
    addTile(i, j, tileSize) {
        ...
        let bonusIcon = null;
        // if(bonusChance>90){
        //     let bonus = this.bonusTypes[this.random.integerInRange(0, this.bonusTypes.length - 1)];
        ...
        this.tiles.add(tile)
        // if(bonusType){
        //     let bonus = game.add.responsiveSprite(0, 0, 'icons',bonusIcon)
        ...
        // tile.events.onInputUp.add((tile, pointer) => this.signals.tileDropped.dispatch(tile, pointer), this);
